...
import { siteEntriesModule } from './src/admin/viteSiteEntries.mjs';
...
export default defineConfig({
  ...
  vite: {
    ...
    // `siteEntriesModule` : module virtuel `virtual:bencat-site-entries`,
    // l'index des entrées lu sur le disque au build / au démarrage du dev,
    // importé par src/admin/cms.ts seulement (sélecteur du bloc Carte et
    // aperçu ; plan 23, T4, D152).
    plugins: [tailwindcss(), adminLibCopy(), siteEntriesModule()],
    ...
    // seulement (plan 21, T1), et pour l'analyseur des blocs `:::` qu'il charge
    // (plan 23 : sans eux, « optimized dependencies changed. reloading » au
    // premier aperçu d'un corps).
    optimizeDeps: {
      include: [
        '@sveltia/cms',
