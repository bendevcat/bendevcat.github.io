...
## D136 · Plan 20 · verify — R19 (2) preview-image race handed to plan 21; R5 instrument substituted   ⚑ à relire
Choice: R19's transient 404s (the preview iframe requesting a draft body's relative images at `/` before Sveltia swaps them to blob URLs) are a preview-rendering defect from plan 19's default preview; they are excluded from plan 20's R19 and become an input of plan 21 (which replaces the preview rendering and must resolve body images without a 404). R5 is judged on the visible glyphs (24×24 boxes, screenshot) because `document.fonts.check` is defeated by Sveltia's own never-loaded CDN `@font-face` · Alternatives: fix the race inside plan 20 (outside its files); park R19 · Reversibility: cheap · Why: plan 20 verification attempt 1; the navigation config cannot cause or fix either.

## D137 · Plan 20 · F1 — `load_config_file: false` inside `config`; R20's `src/admin` line exempts F1's files
Choice: the test-repo board calls `CMS.init({ config })` with a config built from `/admin/config.yml` (same `yaml` options as Sveltia), `backend: { name: 'test-repo' }` and `load_config_file: false` **inside** `config` (Sveltia 0.221 `initCmsConfig` reads it there; the plan's sibling form is ignored); a config load failure logs one error and does not start the test board; R20's "`src/admin` unchanged since base" line exempts F1's own files (`src/admin/cms.ts`, `src/admin/dev/testRepo.ts` + test) · Alternatives: the plan's literal form (warnings stay); fall back to the old merge · Reversibility: cheap · Why: plan 20 R19 (1); orchestrator walk after F1 in a fresh tab on a cold Vite cache: 0 console warning/error, every request 200.

## D138 · Plan 21 · — Preview bodies through the site's own markdown pipeline, with an admin-only copy of `src/lib`
Choice: preview bodies are rendered in the browser by `@astrojs/markdown-remark` with the site's options (Shiki `syntaxTheme`, `rehype-slug`, `rehype-autolink-headings`), shared through a new `src/lib/markdownOptions.mjs` used by `astro.config.mjs` too; `globalThis.process ??= { env: {} }` set in the admin module (the package reads `process.env` unguarded); a small Vite plugin gives the admin its own `?admin` copy of `src/lib` so site pages keep sharing no chunk with the admin (isolation 1/52, site HTML byte-identical); templates get `h`/`createClass` from `window` (set by Sveltia's bundle); images get a `src` only once `getAsset()` yields a `blob:` URL (fixes D136) · Alternatives: the brief's fallback (Sveltia body + site CSS + Shiki); accept shared chunks; hand-copied logic · Reversibility: cheap · Why: plan 21 authoring spike — k9s rendered in headless Brave (29 coloured blocks, 12 linked headings, 103 ms); Node rendering with the same options byte-identical to the built prose of the 5 published articles.

## D139 · Plan 21 · — "Saving an unchanged entry rewrites its file byte for byte" read as three checks   ⚑ à relire
Choice: (a) opening an entry writes nothing (Sveltia 0.221 disables Save on an unchanged entry); (b) the same edit saves identical files before and after the `text` → `code` widget switch; (c) `src/content` has no diff. Keeping Sveltia's own frontmatter re-formatting out of a real edit (a custom file format) is out of scope. If (a) or (b) fails, the plan escalates instead of silently reverting to `text` · Alternatives: a custom serialiser that preserves the original formatting · Reversibility: cheap to decide, expensive to build · Why: plan 19 R14 showed Sveltia re-serialises the whole frontmatter on any save; the brief's criterion targets the widget switch.

## D140 · Plan 21 · — Preview choices   ⚑ à relire
Choice: prompt lines coloured by the site's rule (2–6 `#` + space); a skill's "first file" = the file its page opens on (`defaultFile`); the skill preview also shows its body (« En détail », centre column); section order follows the page for projects and the goal for skills; project banner, tab rows, copy buttons, variable inputs and the article header are omitted; previews in the light theme; `code` widget with `allow_language_selection: false` (`markdown` for prompts and excerpts, `yaml` for snippets); a new unsaved prompt shows `nouveau.md`; `scripts/check-previews.mjs` loads templates through Vite SSR; admin gains Shiki's lazy language chunks (~12 MB, admin only). ⚑ security: the preview iframe keeps Sveltia's `allow-scripts` + `allow-same-origin` sandbox (and its console warning) — not changeable without patching Sveltia · Alternatives: see plan 21 · Reversibility: cheap (sandbox: expensive) · Why: plan 21 authoring.
