...
| R6 | `/transparence-ia` explique les 3 niveaux — `none`/`partial`/`full` **ET** leur signalétique (couleurs des bannières) | In progress | T-C1. La page **rend les 3 bannières réelles** via `AiBanner`, source unique `AI_USAGE_META` : la couleur montrée ne peut pas diverger de l'explication. |
...
| C1 | C | R6 (`/transparence-ia`) · **+ addition n°2** (lien depuis `AiBanner`) | In progress |
