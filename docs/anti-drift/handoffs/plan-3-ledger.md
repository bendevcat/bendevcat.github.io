...
|---|---|---|---|
| R1 | Collection `projects` + schéma Zod (page bundles) — `astro build` valide le schéma **ET** ≥ 2 fiches réelles chargées | Pending | T-A1 (schéma + helpers + tests) puis T-A2 (fiches réelles). Le schéma est repris **verbatim** de la spec de design §3.2 — pas de champ `draft` pour les projets. |
