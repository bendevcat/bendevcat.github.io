...
- **Caught late:** no
- **Status:** rejected
- **User decision:** **Rejetée** le 2026-07-31. L'utilisateur avait bien choisi l'**intention** (option (a), « rendre l'onglet inoffensif », le 2026-07-30), mais le **mécanisme décrit ici est écarté** : il s'est avéré impossible (Sveltia rejette tout `public_folder` global relatif) et **rien n'a jamais été exécuté**. **Remédiation : D09**, qui atteint la même intention par un chemin absolu pointant dans `src/` — approuvée et appliquée. Statut corrigé en Phase Z : la formulation précédente (« approved … mais non réalisable ») n'était aucun des trois statuts autorisés et a été ré-auditée comme `pending-user` par le garde-fou du Lock 4.
