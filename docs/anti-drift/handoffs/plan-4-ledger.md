...
|---|---|---|---|
| R1 | Collections `prompts` + `skills` + schémas Zod (page bundles) — `astro build` valide les 2 schémas **ET** ≥ 2 prompts + ≥ 2 skills réels | Pending | T-A1 (schémas + helpers + tests) puis T-A2 (contenu réel). Schémas repris **verbatim** de la spec de design §3.3 et §3.4 — ni champ en plus, ni champ en moins. `draft` **existe** ici, contrairement à `projects`. |
...
| R7 | Relation `skills ↔ prompts` résolue, aucun `undefined` | Pending | T-A1 (garde-fou `assertEntriesResolved`, déplacé dans `src/lib/references.ts`), T-A2 (données + preuve d'échec par sonde), T-C2 (rendu + preuve d'échec réelle). **Fait établi au Plan 3 : une référence cassée laisse le build VERT tant qu'aucune page ne la résout.** |
