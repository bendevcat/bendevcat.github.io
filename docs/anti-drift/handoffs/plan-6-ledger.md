...
| R12 | Contrastes AA — `body` sur `bg`, `muted` sur `surface`, `accent` sur `accentSoft` ≥ 4.5:1 dans les deux thèmes | Done | **Établie en `8b977f6`, confirmée au rendu en `a508ee5`.** Luminance relative WCAG, `accentSoft` composé *source-over* sur `surface` avant calcul (clair → `rgb(231,240,237)`, sombre → `rgb(25,46,39)`). **6/6 ≥ 4.5:1** — clair : `body`/`bg` **11.79**, `muted`/`surface` **6.33**, `accent`/`accentSoft` **5.61** ; sombre : **12.75**, **7.34**, **8.25**. Recalculés indépendamment par le relecteur, qui retrouve les mêmes chiffres à la décimale. Le relecteur a aussi vérifié le fond alternatif réel (pilule d'accent posée sur `bg`) : **5.10** en clair, **9.21** en sombre — AA également. **Aucune valeur du contrat n'a été ajustée** ; le test de sincérité est que sauter la composition aurait affiché 6.52 au lieu de 5.61, donc un chiffre plus flatteur. **Le contrôle au rendu a révélé un cas que le calcul sur papier ne couvrait pas** : `accentSoft` porte de l'alpha, donc son contraste dépend de ce qu'il y a *derrière*. Posé sur `surface` il reproduit le calcul de la Phase A (8.25 sombre / 5.62 clair) ; posé sur **`bg`** — la pilule du logo dans le header — il donne 9.20 sombre / **5.11** clair. Les six paires passent, mais **5.11:1 est la marge la plus mince de tout le contrat** et elle bougera si un fond clair change. Recalculé indépendamment par le relecteur (composite `rgb(218,230,230)`, ratio 5.104–5.114). **À reprendre en P8**, dont la spec porte déjà « contrastes AA » — en y nommant 5.11 comme le fil déclencheur. **Angle mort du critère, relevé par la revue finale :** R12 et V8 étant des listes **fermées** de trois paires, l'audit ne pouvait pas voir la seule régression de contraste que ce plan ait lui-même causée — les puces de liste de `.prose`, passées à un token à **4.19:1** en clair. Voir **D03**. Une liste fermée est auditable mais pas exhaustive ; c'est le prix payé pour qu'elle soit binaire. |
...
| V2 | Aucun saut de niveau de surface | D1 | Done (`c313362`) — audité **en sombre**, seul thème où un saut est visible : le contrat donne `card` = `surface` = `#FFFFFF` en clair. Les 8 composants sont posés sur `bg` ; aucun saut. **Honnêteté du critère, relevée par la revue finale :** le jeu de test est presque vide à ce stade. `--color-card` et `--color-rail` ne sont consommés par rien, `.card-inner` et `.panel` ne sont portés par aucun élément du site — il n'existe donc aujourd'hui **aucune surface de niveau 3 ou 4 vers laquelle sauter**. V2 passe, mais c'est P7 qui lui donnera de quoi échouer, en introduisant les panneaux de section du §5.3. Un défaut réel corrigé au passage : la puce de statut `archivé` était en `bg-surface` sur une carte déjà `surface` — invisible en clair, corrigée en `bg-chip`. |
...
| V6 | Rayons ∈ {9, 10, 14, 20, 999}px | B1, B2, D1, D2 | **Done (`a508ee5`).** Le gate **repo-wide** ne renvoie aucune ligne. Le relecteur l'a aussi rejoué sur le commit *précédent* pour vérifier l'état « avant » : **31 lignes, toutes dans `src/pages/`** — le compte annoncé est exact. **Six** arbitrages d'usage sont assumés — cette note n'en nommait que deux avant la revue finale, qui a relevé le sous-rapport : les boutons du `Hero` en `rounded-pill` · les lignes de résultat de recherche en `rounded-thumb` (le §3.2 nomme 10px pour « vignette, petite image », pas pour une ligne de texte) · les **quatre `<select>`** de facettes en `rounded-pill` (999px est nommé pour « navigation, statut, catégorie, bouton rond » — une liste déroulante native n'est aucun des quatre) · le bouton `esc` et le `<kbd>` de `/404` · `AiBanner` en `rounded-card` (20px, lu comme « grande carte »). **V6 contrôle l'ensemble des valeurs autorisées, pas leur usage** — ces six passent le gate et restent des lectures, pas des déductions. |
...
- **Accessibilité de la palette Shiki — constat mesuré, hors périmètre du contrat.** Poser
  ...
  **Correction du 2026-09-13, apportée par la revue finale de branche : une phrase de cette note
  était fausse.** Elle concluait « le thème sombre livrait une couleur de syntaxe sous AA avant ce
  plan, et ne le fait plus ». **4.07 reste sous 4.5** : l'amélioration est réelle, la conclusion ne
  l'était pas. Le thème sombre publie toujours une couleur de syntaxe sous AA, moins gravement
  qu'avant. Les chiffres autour de la phrase étaient justes ; seule l'inférence tirée d'eux ne
  l'était pas. **Channel adjudiqué en (b), constat et non déviation**, par un agent à qui la question a été
